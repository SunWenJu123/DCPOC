...
class SequentialTinyImagenet(ContinualDataset):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = MyTinyImagenet(base_path() + 'TINYIMG',
                                 train=True, download=True, transform=self.cifar10_train_transform)
        if self.args.validation:
            train_dataset, test_dataset = get_train_val(train_dataset,
                                                    self.cifar10_test_transform, self.NAME)
        else:
            test_dataset = TinyImagenet(base_path() + 'TINYIMG',
                        train=False, download=True, transform=self.cifar10_test_transform)

        if self.args.featureNet != 'None':
            train, test = getfeature_loader(train_dataset, test_dataset, setting=self)
        else:
            train, test = store_masked_loaders(train_dataset, test_dataset, setting=self)
        return train, test


    def not_aug_dataloader(self, batch_size):
        transform = transforms.Compose([transforms.Resize(224), transforms.ToTensor(), self.normalization_transform])
